<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SummaryReport.aspx.cs" Inherits="Cloudbearing.TimeOffRequestWeb.Pages.SummaryReport" %>
     ...
     <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            function GetListTemplateLevel3(items) {                
                ...
                listtemplate += "   <ul style='margin-left:90px; width:100%;'>";
                var type;
                $.each(items, function (index, item) {
                    type = item.TimeOffType.substring(0, 22)+"...";
                    listtemplate += "<li>";                   
                    listtemplate += "<div class='type'>" + type + "</div>";
                    listtemplate += "<div class='status'>" + item.Status + "</div>";
            ...
            function FillLevel3Details(e, requestedbyid) {
                ...
                $.ajax({
                    type: "POST",
                    url: "SummaryReport.aspx/GetDetails",
                    data: JSON.stringify({ 'year': yearselected, 'parameterkey': 'RequestedBy', 'parametervalue': requestedbyid, 'startdate': StartDate, 'enddate': EndDate, 'starttime': StartTime, 'endtime': EndTime }),
            ...
            function ShowSecLevel(e)
            {
                if (e.parent().hasClass("ui-accordion-header-active ui-state-active")) {
                    e.parent().removeClass("ui-accordion-header-active ui-state-active");
                    if (e.parent().next().hasClass('level3')) {
                        e.parent().next().hide();
                    }
                }
                else {
                    e.parent().addClass("ui-accordion-header-active ui-state-active");
                    if (e.parent().next().hasClass('level3')) {
                        e.parent().next().show();
                    }
                }
                ...
                e.parent().find('.ui-icon').toggleClass('ui-icon-triangle-1-s').toggleClass('ui-icon-triangle-1-e');
                if (e.parent().parent().find('ul').text() == "") {
                    FillLevel3Details(e.find('.requestedby').parent().parent().parent(), e[0].id);
                }
            }
            ...
            function FillLevel2(items) {
                ...
                $.each(items, function (index, item) {
                    ...
                    listtemplate += "<span class='ui-accordion-header-icon ui-icon ui-icon-triangle-1-e'></span><span class='level2header' id='" + item.RequestedByID + "'><span  class='requestedby' >" + item.RequestedBy + "</span><span style='float:right;margin-right:39px;'><b>" + item.TotalHours + "</b></span></span>";
...
<body  style="display:none; overflow:scroll;">
    <form id="form1" runat="server">
        ...
        <div class="container">
             ...
             <div style="float:left; width:10%;"><a onclick="Redirect('Ui.aspx')" class="back-button big page-back"></a></div>
              <div style="float:left; width:90%; margin-left:-50px"><h2>All Time Off Requests <small></small></h2></div>
          </div>
             </div>
        ...
        <asp:Label runat="server" ID="lblerrmsg"  CssClass="errmsg validation"></asp:Label>
            ...
            <div id="Div1" style="display:none;position:absolute;top:50%;left:50%;padding:2px; z-index:3000;">
 ...
 <div id="AccordianParent" style="width:99.5%; padding-left:50px;margin-top:20px;">
              ...
              <div class="divSingleline ui-helper-reset ui-state-default " style="width:8%; padding-right:8px;height:35px;">Requestor</div>
              <div class="divSingleline ui-helper-reset ui-state-default " style="width:13%; padding-right:8px;height:35px;">Type</div>
              ...
              <div class="divSingleline ui-helper-reset ui-state-default " style="width:20%; padding-right:8px; height:35px;">Approver</div>          
              <div class="divSingleline ui-helper-reset ui-state-default " style="width:8.5%; padding-right:8px;height:35px;">Hours</div>  <br />    
